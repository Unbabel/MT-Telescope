...
from metrics.result import PairedResult, BootstrapResult
import numpy as np
    ...
    def score(self, sources, hypothesis, references, **kwargs):
        ...
        if len(new_idx) > 0:       
            ...
            for i, idx in enumerate(new_idx):
                self.cache[(sources[idx], hypothesis[idx], references[idx])] = new_scores[i]

        scores = [self.cache[(s,h,r)] for s, h, r in zip(sources, hypothesis, references)]
        return COMETResult(sum(scores)/len(scores), scores, sources, hypothesis, references)
